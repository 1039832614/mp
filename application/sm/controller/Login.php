	...
	public function updateInfo()
	{
		...
		if($validate->check($data)){
			...
			$res = Db::table('sm_user')
						->where('id',$data['uid'])
						->update($arr);
			if(!empty($data['share_id']) && !empty($data['team_id'])){
				if($data['share_id'] !== 0 && $data['team_id'] !== 0)
				{
					//构建邀请成员信息表入库数据
					$inv = [
						'team_id' => $data['team_id'],
						'sm_header_id' => $data['share_id'],
						'sm_member_id' => $data['uid'],
						'type' => 1
					];
					$invite = Db::table('sm_team_invite')
								->strict(false)
								->insert($inv);
				}
			}
...
//					if($count < 1) {
    ...
    private function unifiedorder($lastId,$openid) {  
        ...
        $parameters = array(  
            ...
            'notify_url' => 'https://mp.ctbls.com/sm/Login/notify', 
            'trade_type' => 'JSAPI',
	...
	public function notify(){
		...
		$sign = $this->wx->getSign($data);
			...
			if (($sign===$data_sign) && ($data['return_code']=='SUCCESS') && ($data['result_code']=='SUCCESS')){
					...
					Db::table('sm_area')
					->where('id',$attach['cid'])
					->update(['transaction_id'=>$result['transaction_id'],'pay_status'=>1,'pay_time'=>time()]);
					//获取用户id和身份类型
